...
{
	// Add the first device as SR300 color sensor
	imaqkit::IDeviceInfo* deviceInfo =
		hardwareInfo->createDeviceInfo(1, "SR300 color sensor");
	setSR300ColorFormat(deviceInfo);
	hardwareInfo->addDevice(deviceInfo);

	// create the second device as SR300 depth sensor
	deviceInfo =
		hardwareInfo->createDeviceInfo(2, "SR300 depth sensor");
	setSR300DepthFormat(deviceInfo);
	hardwareInfo->addDevice(deviceInfo);
}
void getDeviceAttributes(const imaqkit::IDeviceInfo* deviceInfo,
	...
	if (devID == 1) {
		...
		sourceContainer->addAdaptorSource("input1", 1);
		//TODO: need to check the fx / fy values for SR300
		hProp = devicePropFact->createDoubleProperty("SR300FocalLengthX", 475.0);
		...
		hProp = devicePropFact->createDoubleProperty("SR300FocalLengthY", 475.0);
		// Set the property to read-only while running.
		devicePropFact->setPropReadOnly(hProp, imaqkit::propreadonly::WHILE_RUNNING);
		// Add the property to the device-specific property container.
		devicePropFact->addProperty(hProp);
		devicePropFact->setVisibility(hProp, imaqkit::visibility::INVISIBLE);
		hProp = devicePropFact->createDoubleProperty("SR300PrincipalPointX", 327.0);
		// Set the property to read-only while running.
		devicePropFact->setPropReadOnly(hProp, imaqkit::propreadonly::WHILE_RUNNING);
		// Add the property to the device-specific property container.
		devicePropFact->addProperty(hProp);
		devicePropFact->setVisibility(hProp, imaqkit::visibility::INVISIBLE);
		hProp = devicePropFact->createDoubleProperty("SR300PrincipalPointY", 246.0);
		// Set the property to read-only while running.
		devicePropFact->setPropReadOnly(hProp, imaqkit::propreadonly::WHILE_RUNNING);
		// Add the property to the device-specific property container.
		devicePropFact->addProperty(hProp);
