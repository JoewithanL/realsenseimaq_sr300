...
DWORD WINAPI RealSenseAdaptor::acquireThread(void* param) 
{
	...
	while (GetMessage(&msg, NULL, 0, 0) > 0) {
		...
		case WM_USER:
		{
			if (camera_id == 1)
			{
				sendsr300colorframe(adaptor);
			}
			else if (camera_id ==2)
			{
				sendsr300depthframe(adaptor);
			}
			break;
		} //switch-case WM_USER
...
void RealSenseAdaptor::sendsr300colorframe(RealSenseAdaptor* adaptor)
	{
...
void RealSenseAdaptor::sendsr300depthframe(RealSenseAdaptor* adaptor)
{
